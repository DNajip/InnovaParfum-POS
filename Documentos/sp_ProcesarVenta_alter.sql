...
Pendiente
    ...
    IF @TotalPag
adoNio < @TotalVentaNio
    BEGIN
        SET @SaldoPendiente = @TotalVentaNio - @TotalPagadoNio;
    END

    -- 4. Insertar Venta
    INSERT INTO VEN.VENTAS (ID_TURNO, ID_USUARIO, ID_PERSONA, FECHA_VENTA, TASA_CAMBIO_USD, SUBTOTAL_NIO, DESCUENTO_NIO, TO
TAL_NIO, ANULADA, ID_TIPO_VENTA, ID_CONDICION_PAGO)
...
o = DIAS_CREDITO 
        ...
        INSERT INTO VEN.CREDITOS (ID_VENTA, ID_PERSONA, MONTO_
ORIGINAL, SALDO_PENDIENTE, FECHA_CREDITO, FECHA_VENCIMIENTO, ESTADO)
   ...
   UPDATE ADM.CLIENTES_CREDITO 
    ...
    DECLARE @VueltoTotalNio DECIMAL(12,2) = CASE WHEN @TotalPagadoNi
o > @TotalVentaNio THEN @TotalPagadoNio - @TotalVentaNio ELSE 0 END;
    ...
    BEGIN
        ...
        BEGIN
            DECLARE @NotaAuto NVARCHAR(200) = 'Vuelto de C$ ' + CAST(@
VueltoTotalNio AS NVARCHAR(20)) + ' entregado en efectivo por sobrepago en método no-efectivo.';
            UPDATE VEN.VENTAS SET OBSERVACION = ISNULL(OBSERVACION + ' | ', '') + @NotaAuto WHERE ID_VENTA = @IdVenta;
        END
    END

    -- Insertar r
egistros de pagos con detalle de recibido/vuelto
...
p;
    ...
    BEGIN
        ...
        UPDATE 
VEN.PAGOS SET VUELTO_NIO = @VueltoTotalNio WHERE ID_PAGO = @UltimoIdPago;
    END

    -- 6. Actualizar Saldos de Caja (Turno)
    UPDATE T SET 
        T.TOTAL_EFECTIVO_NIO += (ISNULL((SELECT SUM(CAST(JSON_VALUE(pj.[value], '$.MontoEnNio') AS DECIMAL(12,
2))) FROM OPENJSON(@PaymentsJson) pj JOIN CAT.METODOS_PAGO mp ON mp.ID_METODO = CAST(JSON_VALUE(pj.[value], '$.IdMetodoPago') AS INT) JOIN CAT.MONEDAS m ON m.ID_MONEDA = mp.ID_MONEDA WHERE mp.NOMBRE LIKE '%EFECTIVO%' AND m.CODIGO = 'NIO'), 0) - @VueltoTot
alNio),
        T.TOTAL_EFECTIVO_USD += ISNULL((SELECT SUM(CAST(JSON_VALUE(pj.[value], '$.Monto') AS DECIMAL(12,2))) FROM OPENJSON(@PaymentsJson) pj JOIN CAT.METODOS_PAGO mp ON mp.ID_METODO = CAST(JSON_VALUE(pj.[value], '$.IdMetodoPago') AS INT) JOIN CAT.
MONEDAS m ON m.ID_MONEDA = mp.ID_MONEDA WHERE mp.NOMBRE LIKE '%EFECTIVO%' AND m.CODIGO = 'USD'), 0),
        T.TOTAL_TARJETA += ISNULL((SELECT SUM(CAST(JSON_VALUE(pj.[value], '$.MontoEnNio') AS DECIMAL(12,2))) FROM OPENJSON(@PaymentsJson) pj JOIN CAT.METO
DOS_PAGO mp ON mp.ID_METODO = CAST(JSON_VALUE(pj.[value], '$.IdMetodoPago') AS INT) WHERE mp.NOMBRE LIKE '%TARJETA%'), 0),
        T.TOTAL_TRANSFERENCIA += ISNULL((SELECT SUM(CAST(JSON_VALUE(pj.[value], '$.MontoEnNio') AS DECIMAL(12,2))) FROM OPENJSON(@Pa
ymentsJson) pj JOIN CAT.METODOS_PAGO mp ON mp.ID_METODO = CAST(JSON_VALUE(pj.[value], '$.IdMetodoPago') AS INT) WHERE mp.NOMBRE LIKE '%TRANSFERENCIA%'), 0),
        T.TOTAL_VENTAS_NIO += @TotalPagadoNio, -- En POS solo sumamos lo realmente cobrado para cu
adre
        T.TOTAL_VENTAS_USD += (@TotalPagadoNio / @TasaCambioUsd)
    FROM CAJA.TURNOS T WHERE T.ID_TURNO = @IdTurno;

    -- 7. Procesar Items y Garantías (Iteramos por unidad para precisión total)
    DECLARE @IdProducto INT, @DescSnap NVARCHAR(20
0), @UnitPrice DECIMAL(12,2), @IdPeriodo INT, @Meses INT;
    ...
    SELECT 
        ...
        CAS
T(JSON_VALUE(i.[value], '$.UnitPrice') AS DECIMAL(12,2)),
        CAST(JSON_VALUE(d.[value], '$.IdPeriodoGarantia') AS INT),
        PG.MESES
    FROM OPENJSON(@ItemsJson) AS i
    CROSS APPLY OPENJSON(i.[value], '$.Details') AS d
    JOIN CAT.PERIODOS_GA
RANTIA PG ON PG.ID_PERIODO = CAST(JSON_VALUE(d.[value], '$.IdPeriodoGarantia') AS INT);
 ...
 = 'Stock agotado para ' + @DescSnap + '. No se puede completar la venta.';
...
Stock
        ...
        IF @Meses > 0 SET @FechaVence = DATEADD(MONTH, @Meses, @Fech
aActual);
        ...
        VALUES 
(@IdVenta, @IdProducto, @DescSnap, 1, @UnitPrice, @UnitPrice, @IdPeriodo, @FechaVence);
        SET @IdDetalle = SCOPE_IDENTITY();

        -- F. Registrar Garantía formal en GAR.GARANTIAS si hay cliente y meses > 0
        IF @IdPersona IS NOT NULL AND 
@Meses > 0
        BEGIN
            ...
            VALUES (@IdDetalle, @IdPersona, @IdProducto, @Meses, CAST(@FechaActual
 AS DATE), @FechaVence, 'ACTIVA');
        END
    ...
    COMMIT TRANSACTION;
    SELECT * FROM V
EN.VENTAS WHERE ID_VENTA = @IdVenta;
END;
