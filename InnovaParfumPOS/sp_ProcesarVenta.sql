...
totales
    ...
    IF @TotalPagadoBase < @TotalVentaNio
    BEGIN
        SET @SaldoPendiente = @TotalVentaNio - @TotalPagadoBase;
    END
    ...
    DECLARE @VueltoTotalBase DECIMAL(12,2) = CASE WHEN @TotalPagadoBase > @TotalVentaNio THEN @TotalPagadoBase - @TotalVentaNio ELSE 0 END;
    ...
    BEGIN
        ...
        UPDATE VEN.PAGOS SET VUELTO_NIO = @VueltoTotalBase WHERE ID_PAGO = @UltimoIdPago;
    END

    -- 6. Actualizar Saldos de Caja (Turno)
    DECLARE @VueltoParaNio DECIMAL(12,2) = CASE WHEN @MonedaVuelto = 'NIO' THEN (@VueltoTotalBase * @TasaCambioUsd) ELSE 0 END;
    DECLARE @VueltoParaUsd DECIMAL(12,2) = CASE WHEN @MonedaVuelto = 'USD' THEN @VueltoTotalBase ELSE 0 END;
    ...
    UPDATE T SET 
        ...
        T.TOTAL_VENTAS_NIO += ISNULL((SELECT SUM(CAST(JSON_VALUE(p.[value], '$.MontoEnNio') AS DECIMAL(12,2))) FROM OPENJSON(@PaymentsJson) AS p), 0),
        T.TOTAL_VENTAS_USD += (ISNULL((SELECT SUM(CAST(JSON_VALUE(p.[value], '$.MontoEnNio') AS DECIMAL(12,2))) FROM OPENJSON(@PaymentsJson) AS p), 0) / NULLIF(@TasaCambioUsd, 0))
    FROM CAJA.TURNOS T WHERE T.ID_TURNO = @IdTurno;

    -- 7. Procesar Items y Garantías (Iteramos por unidad para precisión total)
    DECLARE @IdProducto INT, @DescSnap NVARCHAR(200), @UnitPrice DECIMAL(12,2), @IdPeriodo INT, @Meses INT, @EsRegalia BIT;
...
FOR
    SELECT 
        ...
        CAST(JSON_VALUE(d.[value], '$.IdPeriodoGarantia') AS I
NT),
        PG.MESES,
    ...
    JOIN CAT.PERIODOS_GARANTIA PG ON PG.ID_PERIODO = CAST(JSON_VALUE
(d.[value], '$.IdPeriodoGarantia') AS INT);

    OPEN detail_cursor;
    FETCH NEXT FROM detail_cursor INTO @IdProducto, @DescSnap, @UnitPrice, @IdPeriodo, @Meses, @EsRegalia;
    ...
    BEGIN
        ...
        IF @StockActual < 1
        BEGIN
            DECLARE @ErrorMsg NVARCHAR(300) = 'Stock agotado para ' + @Desc
Snap + '. No se puede completar la venta.';
        ...
        -- B. Reducir Stock
        UPDATE INV.PRODUCT
OS SET STOCK_ACTUAL = STOCK_ACTUAL - 1 WHERE ID_PRODUCTO = @IdProducto;
        ...
        -- D. Inserta
r VENTA_DETALLE (Cantidad = 1 por fila)
        ...
        VALUES (@IdVenta, @IdProduc
to, @DescSnap, 1, @UnitPrice, @UnitPrice, @IdPeriodo, @FechaVence, @EsRegalia);
        SET @IdDetalle = SCOPE_IDENTITY();

        -- F. Registrar Garantía formal en GAR.GARANTIAS si hay cliente y meses > 0
        IF @IdPersona IS NOT NULL AND @Meses >
 0
        BEGIN
            ...
            VALUES (@IdDetalle, @IdPersona, @IdProducto, @Meses, CAST(@FechaActual AS DATE
), @FechaVence, 'ACTIVA');
        END
    ...
    COMMIT TRANSACTION;
    SELECT * FR
OM VEN.VENTAS WHERE ID_VENTA = @IdVenta;
END;
