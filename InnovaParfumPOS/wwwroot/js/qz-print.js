...
window.qzPrintAbono = async (abono, printerName) => {
    try {
        ...
        data = data.concat([
            ...
            "------------------------------------------------\n",
            left,
            `Credito #: ${abono.numeroCredito}\n`,
            `Factura:   ${abono.facturaReferencia}\n`,
            `Fecha:     ${abono.fecha}\n`,
            `Cliente:   ${abono.cliente}\n`,
            "------------------------------------------------\n",
            right,
            `Saldo Anterior:    ${abono.simboloMoneda || "$"} ${abono.saldoAnterior.toFixed(2)}\n`,
            `Monto Abonado:     ${abono.simboloMoneda || "$"} ${abono.montoAbonado.toFixed(2)}\n`,
            boldOn,
            `NUEVO SALDO:       ${abono.simboloMoneda || "$"} ${abono.nuevoSaldo.toFixed(2)}\n`,
            boldOff,
            center,
